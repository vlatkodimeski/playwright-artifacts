        ...
        if (fs.existsSync(xmlFilePath)) {
            ...
            parseString(xmlContent, (err, result) => {
                ...
                if (result && result.testsuites && result.testsuites.testsuite) {
                    for (const suite of result.testsuites.testsuite) {
                        if (suite.testcase) {
                            for (const testcase of suite.testcase) {
                                // Remove baseURL attribute from testcase
                                if (testcase.$ && testcase.$.baseURL) {
                                    delete testcase.$.baseURL;
                                }

                                // Add CDATA inside failure elements
                                if (testcase.failure) {
                                    testcase.failure[0]._ = `<![CDATA[${testcase.failure[0]._}]]>`;
                                }
                            }
                        }
                    }
                    ...
                    console.log('Updated XML report with properly formatted CDATA content');
                }
            });
        } else {
